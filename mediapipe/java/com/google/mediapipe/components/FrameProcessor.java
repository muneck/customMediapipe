...
public class FrameProcessor implements TextureFrameProcessor, AudioDataProcessor {
  ...
  public List<LandmarkProto.NormalizedLandmarkList> multiFaceLandmarks;
  ...
  /**
    * used for MUNECK API
    * auto implement a face mesh processor that works only with face_mesh_gpu.pbtxt
    * @param context an Android {@link Context}.
   * @param graphName the name of the file containing the binary representation of the graph.
   * @param inputStream the graph input stream that will receive input video frames.
   * */
  public FrameProcessor(Context context, long parentNativeContext) {
    Log.v(TAG, "MediaPipe FrameProcessor constructor Start!");
    // only for specific use
    String graphName = "face_mesh_mobile_gpu.binarypb";
    String inputStream = "input_video";
    String outputStream = "output_video";
    multiFaceLandmarks = new ArrayList<>();
    try {
      initializeGraphAndPacketCreator(context, graphName);
      addVideoStreams(parentNativeContext, inputStream, outputStream);
    } catch (MediaPipeException e) {
      // TODO: do not suppress exceptions here!
      Log.e(TAG, "MediaPipe error when init Graph and Packet: ", e);
    }
    //flip the surface(at current time this is null) (!DONT need as we have no output)
//    this.getVideoSurfaceOutput().setFlipY(true);

    //set NUM_FACE
    AndroidPacketCreator packetCreator = this.getPacketCreator();
    Map<String, Packet> inputSidePackets = new HashMap<>();
    inputSidePackets.put("num_faces", packetCreator.createInt32(1));
    this.setInputSidePackets(inputSidePackets);

    //add callback
    this.addPacketCallback(
            "multi_face_landmarks",
            (packet) -> {
              Log.v(TAG, "Received multi face landmarks packet. Set to multiFaceLandmarks");
              multiFaceLandmarks = PacketGetter.getProtoVector(packet, LandmarkProto.NormalizedLandmarkList.parser());
//              Log.v(TAG,
//                      "[TS:"
//                              + packet.getTimestamp()
//                              + "] "
//                              + getMultiFaceLandmarksDebugString(multiFaceLandmarks));
    ...
    Log.println(7,"FrameProcessor","Test0 MUNECK_API init success!");
  }

  public float mediapipe_api_get_turn(){
     ...
     */
    List<LandmarkProto.NormalizedLandmark> landmarksList = multiFaceLandmarks.get(0).getLandmarkList();
    float turn = landmarksList.get(33).getZ() - landmarksList.get(263).getZ();
//    float tilt = Math.atan(landmarksList.get(33).getY() - landmarksList.get(263).getY()) / (landmarksList.get(33).getX() - landmarksList.get(263).getX()) + Math.PI/2.0;
//    float nod = landmarksList.get(1).getZ() - landmarksList.get(168).getZ();
    return turn;
  }
  public float mediapipe_api_get_tilt(){
    if (multiFaceLandmarks.isEmpty()) {
      return 0;
    }
    List<LandmarkProto.NormalizedLandmark> landmarksList = multiFaceLandmarks.get(0).getLandmarkList();
//    float turn = landmarksList.get(33).getZ() - landmarksList.get(263).getZ();
    float bias = (float)(Math.PI/2.0);
    float tilt = (float)Math.atan(landmarksList.get(33).getY() - landmarksList.get(263).getY()) / (landmarksList.get(33).getX() - landmarksList.get(263).getX()) + bias;
//    float nod = landmarksList.get(1).getZ() - landmarksList.get(168).getZ();
    return tilt;
  }
  public float mediapipe_api_get_nod(){
    if (multiFaceLandmarks.isEmpty()) {
      return 0;
    }
    /**
...
//    float tilt = Math.atan(landmarksList.get(33).getY() - landmarksList.get(263).getY()) / (landmarksList.get(33).getX() - landmarksList.get(263).getX()) + Math.PI/2.0;
  ...
  public boolean test_func(boolean flag){ Log.println(7,"FrameProcessor","Test1 success! flag=" + Boolean.toString(flag)); return (!flag);}
  ...
  public void setUseImage(boolean use) {
    useImage = use;
    Log.println(7,"FrameProcessor","test2 success!");
  }
