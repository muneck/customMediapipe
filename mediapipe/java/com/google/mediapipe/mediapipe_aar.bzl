...
def proto_java_src_generator(name, proto_src, java_lite_out, srcs = []):
    native.genrule(
        name = name + "_proto_java_src_generator",
        srcs = srcs + [
            "@com_google_protobuf//:well_known_protos",
...
def android_proto_library(name, protoname, proto_src, java_lite_out, srcs = []):
    proto_java_src_generator(protoname, proto_src, java_lite_out, srcs)
    android_library(
      name = name,
      srcs = [
         java_lite_out,
