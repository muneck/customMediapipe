...
MPFaceMeshDetector::InitFaceMeshDetector(int numFaces,
  ...
  ASSIGN_OR_RETURN(
	  mediapipe::OutputStreamPoller face_detection_poller,
	  graph.AddOutputStreamPoller(kOutputStream_faceDetect));
  ...
  face_detection_poller_ptr =
	  std::make_unique<mediapipe::OutputStreamPoller>(
		  std::move(face_detection_poller));
...
MPFaceMeshDetector::DetectFacesWithStatus(const cv::Mat &camera_frame,
  ...
  *numFaces = 0;
  ...
  // Get face count.
  mediapipe::Packet face_detection;
  if (!face_detection_poller_ptr ||
	  !face_detection_poller_ptr->Next(&face_detection)) {
